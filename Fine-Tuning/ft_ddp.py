...
import json
with open('Finetuning_images.json','r')as f:
    image_paths=json.load(f)
with open('Finetuning_captions.json','r')as f:
    answers=json.load(f)
with open('templates.json','r')as f:
    prompts=json.load(f)

image_paths=['./2nd_exp/2nd_FT_images/'+img for img in image_paths]
...
while epoch < num_epochs:
    ...
    if epoch != 0 :
        ...
        if master_process:
            save_path = f"experiments/NoDecay/all_samples/lora_checkpoint_{epoch}"
            ...
            print(f'gpu {ddp_local_rank} saved lora checkpoint {epoch}')
